...
// ── Poll ───────────────────────────────────────────────────────────────────
socket.on("player:poll", ({ question, options }) => {
  document.getElementById("p-poll-question").textContent = question;
  const grid = document.getElementById("p-poll-grid");
  const colors = ["color-0", "color-1", "color-2", "color-3", "color-0", "color-1"];
  const shapes = ["▲", "◆", "●", "■", "★", "⬟"];
  grid.innerHTML = options.map((opt, i) =>
    `<button class="answer-btn ${colors[i]}" onclick="submitPollVote(${i}, this)">
...
function submitPollVote(index, btn) {
  if (btn.disabled) return;
  document.querySelectorAll("#p-poll-grid .answer-btn").forEach(b => b.disabled = true);
  btn.style.outline = "4px solid #fff";
  socket.emit("player:poll_vote", { optionIndex: index });
}

socket.on("player:poll_voted", () => {
  showScreen("screen-poll-voted");
});

socket.on("poll:results", ({ question, results }) => {
  const container = document.getElementById("p-poll-results");
  ...
  container.innerHTML = results.map(r =>
    `<div class="poll-result-row">
       ...
       <div class="poll-result-bar-wrap">
         ...
         <span class="poll-result-count">${r.votes}</span>
       </div>
     </div>`
  ).join("");
